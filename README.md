...
After a full day, I have the first bits of the ui done. What remains is to draw the cat and display the text. Because the cat is a complicated figure to draw with pixels and lines, I'm probably going to either write a python code that outputs the exact code to recreate it, or use a matrix to show 1s and 0s based on if the pixels are light up, and using that displaying the cat.
